...
import { LogOut, Plus, Trash2, Edit2, Save, X, Eye, EyeOff, Upload, Users, Images } from 'lucide-react';
...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  const [activeTab, setActiveTab] = useState<'transformations' | 'leads'>('transformations');
  ...
  async function updateLeadStatus(leadId: string, newStatus: string) {
    ...
    try {
      ...
      // No need to fetch again, just keep the optimistic update
    } catch (error: any) {
      console.error('Error updating lead status:', error);
      // Already reverted above, just show error
    }
  }
  ...
  return (
    <div className="min-h-screen bg-accent/20 font-sans">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          ...
          <div className="flex gap-2 mt-4 border-b border-border">
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 font-medium transition-all border-b-2 ${activeTab === 'transformations'
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 font-medium transition-all border-b-2 ${activeTab === 'leads'
                ? 'text-primary border-primary'
                : 'text-muted-foreground border-transparent hover:text-foreground'
                }`}
            >
              <Users className="w-4 h-4" />
              Leads {leads.length > 0 && <span className="bg-primary text-white text-xs px-1.5 py-0.5 rounded-full">{leads.length}</span>}
            </button>
          </div>
        </div>
      </nav>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        {activeTab === 'leads' ? (
          <LeadsSection leads={leads} onStatusUpdate={updateLeadStatus} />
        ) : (
          <>
