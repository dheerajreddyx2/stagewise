...
export default function FeaturedTransformations() {
    ...
    async function fetchTransformations() {
        try {
            const { data, error } = await supabase
                ...
                .limit(3); // Only fetch first 3 for featured section
    ...
    if (loading) {
        return (
            <section className="py-16 px-6 bg-gradient-to-br from-purple-50/40 to-pink-50/40">
                <div className="max-w-6xl mx-auto text-center">
                    <div className="text-muted-foreground animate-pulse">Loading transformations...</div>
                </div>
            </section>
        );
    }
                ...
                {transformations.length > 0 && (
                    <div className="text-center mt-12">
                        <button
                            ...
                            className="inline-flex items-center gap-2 px-8 py-4 bg-white/80 backdrop-blur-sm text-purple-600 font-semibold rounded-full hover:shadow-lg hover:scale-105 transition-all duration-300 border-2 border-purple-300/50 hover:border-purple-400 group"
                        >
